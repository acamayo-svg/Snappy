...
**Imagen del producto:** la tabla `productos` tiene columna `imagen` (URL). El establecimiento puede **subir una imagen** desde su dispositivo: **POST /api/productos/subir-imagen** (multipart, campo `imagen`). Formatos: JPEG, PNG, GIF, WebP; máximo 30 MB. Las imágenes se guardan en `servidor/public/uploads/` y se sirven en **GET /uploads/nombre-archivo**. La ruta crea la carpeta automáticamente si no existe.
